 ...
 * @author Higor
 */
public class TesteBongoJuntos {
    private Server server;
    
    public TesteBongoJuntos() {
    ...
    public void play(int time, int loops){
        ...
        for (int i = 0; i < loops; i++) {
            ...
            oscMessage = new OSCMessage("/server/bongobot/playBongoTogetherDef", l);

            this.server.forwardMessage(oscMessage);
            System.out.println("playJuntosDef [id="+i+", RT="+relativeTime+"]");
            relativeTime += time; 
    ...
    public static void main(String[] args) {
        TesteBongoJuntos c = new TesteBongoJuntos();
        while(true){
